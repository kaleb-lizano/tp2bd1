...
async function verificarBloqueoLogin(conexion, ip) {
...
async function login(req, res, next) {
  try {
    ...
    const { intentosFallidos, bloqueadoReciente } = await verificarBloqueoLogin(conexion, ip);

    if (intentosFallidos > 5 || bloqueadoReciente) {
      await registrarEvento(3, "", 1, ip);
    ...
    if (!usuario) {
      ...
      await registrarEvento(2, `${numIntento}. Código: 50001`, 1, ip);
    ...
    if (usuario.Password !== password) {
      ...
      await registrarEvento(2, `${numIntento}. Código: 50002`, usuario.Id, ip);
      return res.status(401).json({ errorCode: 50002, message: errorMsg });
    }

    await registrarEvento(1, "Exitoso", usuario.Id, ip);
...
async function logout(req, res, next) {
  try {
    const { idUsuario, ip } = req.body;
    await registrarEvento(4, "", idUsuario, ip);
